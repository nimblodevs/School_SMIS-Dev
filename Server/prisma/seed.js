...
async function main() {
    const passwordHashes = Object.fromEntries(
        await Promise.all(
            Object.entries(PASSWORDS).map(async ([key, value]) => [
                key,
    ...
    for (const [index, [firstName, lastName, gender]] of extraStudents.entries()) {
        ...
        const seededStudent = await prisma.student.upsert({
            where: {
                schoolId_admissionNo: { schoolId: school.id, admissionNo: `ADM-2026-${number}` },
            },
            update: {
                userId: studentUser.id,
                firstName,
        ...
        await prisma.studentParent.upsert({
            where: {
                studentId_parentId: { studentId: seededStudent.id, parentId: seededParent.id },
            },
    ...
    for (const seededStudent of seededStudents) {
        ...
        await prisma.payment.create({
            data: {
                ...
                method: 'MPESA',
                status: 'COMPLETED',
                mpesaReceipt: `DEMO${seededStudent.admissionNo.replaceAll('-', '')}`,
                mpesaPhone: '+254700000001',
                reference: `PAY-${seededStudent.admissionNo}`,
